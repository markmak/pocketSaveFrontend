...
import { scaleTime, extent, line, scaleLinear, max } from "d3";
...
function SavingProgressChart({ data }) {
  const { createdAt, targetSavingPeriod, savingRecord, amount } = data;
  const [filter, setFilter] = useState("");
  const [chartType, setChartType] = useState("cumulative");
  const [tooltipValue, setTooltipValue] = useState("");

  savingRecord.forEach((data) => {
  ...
  const createMonthList = (startDate, endDate) => {
    const monthList = [startDate];
    while (monthList.at(-1) !== endDate) {
  ...
  if (!filter.startDate) {
    timeDomain[0] = createdAt.substring(0, 7);
  }
  const fullMonthList = useRef(
    createMonthList(createdAt.substring(0, 7), timeDomain[1])
  );
  const cumulativeData = useMemo(() => {
    const binnedData = savingRecord.reduce((a, c) => {
      const { date, amount } = c;
  ...
  }, [savingRecord, fullMonthList]);

  const monthList = createMonthList(timeDomain[0], timeDomain[1]);
  ...
  const yMaxData = max(processedData, (data) => data.amount);
  const yMax =
    chartType === "cumulative"
      ? amount > yMaxData
        ? amount
        : yMaxData
      : Math.ceil(amount / targetSavingPeriod) > yMaxData
      ? Math.ceil(amount / targetSavingPeriod)
      : yMaxData;
    ...
    <>
      ...
      <div className="filter-container">
        <div className="single-filter">
          <label htmlFor="saving-progress-filter-start-date">From:</label>
          <input
            type="date"
            name="startDate"
            className="input-style-bottom"
            id="saving-progress-filter-start-date"
            max={today}
            onChange={filterHandle}
          />
        </div>
        <div className="single-filter">
          <label htmlFor="saving-progress-filter-end-date">To:</label>
          <input
            type="date"
            name="endDate"
            className="input-style-bottom"
            id="saving-progress-filter-end-date"
            max={today}
            onChange={filterHandle}
          />
        </div>
      </div>
      <div className="saving-progress-chart">
        {filteredData.length > 0 ? (
            ...
            <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
              ...
              {chartType === "cumulative" ? (
                ...
                <line
                  ...
                  x1={margin.left}
                  x2={svgWidth - margin.right}
                  y1={yScale(Math.ceil(amount / targetSavingPeriod))}
                  y2={yScale(Math.ceil(amount / targetSavingPeriod))}
                  onMouseMove={(e) => {
                    const { pageX, pageY } = e;
                    setTooltipValue({
                      date: "Mothly saving target",
                      amount: Math.ceil(amount / targetSavingPeriod),
                      x: pageX - 20,
                      y: pageY + 20,
                    });
                  }}
              ...
              <path
                ...
                className="saving-progress-path"
                fill="none"
              />
          ...
          <div className="no-record">No saving record in the given period</div>
        )}
      </div>
    </>
  );
